...
- (id)init {
    if (self = [super init]) {
        //1.获取数据库文件app.db的路径
        NSString *filePath = [self getFileFullPathWithFileName:@"applicationColletion"];
        //2.创建database
...
- (void)insertModel:(id)model recordType:(NSString *)type {
    DetailsModel *appModel = (DetailsModel *)model;
...
- (NSArray *)readModelsWithRecordType:(NSString *)type{
    ...
    while ([rs next]) {
        //把查询之后结果 放在model
        DetailsModel *appModel = [[DetailsModel alloc] init];
